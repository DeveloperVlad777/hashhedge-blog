...
2. Файл содержит ТОЛЬКО блок `.t-article-wrapper` — без `<!DOCTYPE html>`, `<html>`,
   `<head>` и `<body>`. Head не нужен: блок вставляется в Tilda, которая сама
   управляет структурой страницы.
